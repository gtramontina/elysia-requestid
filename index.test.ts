...
it("forwards the request ID header when it is available", async () => {
	const response = await new Elysia()
		.use(requestID({ uuid: () => "some-uuid" }))
		.get("/", () => "dummy")
		.handle(
...
it("allows the request ID header to be configured", async () => {
	const response = await new Elysia()
		.use(requestID({ uuid: () => "some-uuid", header: "X-Custom-Request-ID" }))
		.get("/", () => "dummy")
		.handle(new Request("https://dummy.com/"));
...
it("forwards custom request ID when it is available", async () => {
	const response = await new Elysia()
		.use(requestID({ uuid: () => "some-uuid", header: "X-Custom-Request-ID" }))
		.get("/", () => "dummy")
		.handle(
...
it("provides an accessor to the request ID", async () => {
	const response = await new Elysia()
		.use(requestID({ uuid: () => "some-uuid" }))
